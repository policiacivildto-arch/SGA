...
function CadLotacoes() {
  ...
  const [form, setForm] = useState({ depto: '', nome: '', cidade: '', resp: '', tel: '', end: '' });
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setForm({ depto: '', nome: '', cidade: '', resp: '', tel: '', end: '' });
  ...
  return (
    <>
      ...
      <div className="card">
        <div className="search-bar"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="🔍 Buscar por depto, nome, cidade..." style={{ width: '100%' }} /></div>
        <div className="table-wrap">
          <table>
            <thead><tr><th>Departamento</th><th>Nome</th><th>Cidade</th><th>Responsável</th><th>Telefone</th><th>Ações</th></tr></thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.id}>
                  <td><b>{row.depto}</b></td><td>{row.nome}</td><td>{row.cidade}</td><td>{row.resp || '—'}</td><td>{row.tel || '—'}</td>
